...
class SimpleDataSet(Dataset):
    def __init__(self, config, mode, logger, seed=None):
        ...
        if isinstance(label_file_list, str):
            ...
            if isinstance(label_file_list[0], str):
                label_file_list = [label_file_list]
        data_source_num = sum([len(label_files) for label_files in label_file_list])
        ratio_lists = dataset_config.get("ratio_list", 1.0)
        if isinstance(ratio_lists, (float, int)):
            ratio_lists = [[float(ratio_lists)]* int(data_source_num)] 
        else:
            if isinstance(ratio_lists[0], (float, int)):
                ratio_lists = [ratio_lists]

        assert sum([len(
            ratio_list
        ) for ratio_list in ratio_lists]) == data_source_num, "The length of ratio_list should be the same as the file_list."
        self.data_dirs = dataset_config['data_dir']
        if isinstance(self.data_dirs, str):
            self.data_dirs = [self.data_dirs]
        self.do_shuffle = loader_config['shuffle']
        self.seed = seed
        logger.info("Initialize indexs of datasets:%s" % label_file_list)
        self.data = self.get_image_info_list(label_file_list, ratio_lists)
        ...
        self.ext_op_transform_idx = dataset_config.get("ext_op_transform_idx",
                                                       2)
        self.need_reset = True in [[x < 1 for x in ratio_list] for ratio_list in ratio_lists]
    ...
    def replace_symbol(self, txt):
        org_sym = ['（', '）', '～','Ⅴ', 'Ⅰ','IV','ⅠⅤ','Ⅵ','Ⅶ','III','：','，','φ',' ','①','②','③','④','‘']
        new_sym = ['(', ')', '~','V', 'I', 'Ⅳ', 'Ⅳ', 'VI','VII','Ⅲ',':',',','Φ','','1','2','3','4','\'']
        assert len(org_sym)==len(new_sym)
        for o, n in zip(org_sym, new_sym):
            txt = txt.replace(o,n)
        return txt
    
    def get_image_info_list(self, file_lists, ratio_list):
        datas = []
        for i, file_list in enumerate(file_lists):
            for idx, file in enumerate(file_list):
                with open(file, "rb") as f:
                    lines = f.readlines()
                    if self.mode == "train" or ratio_list[i][idx] < 1.0:
                        random.seed(self.seed)
                        lines = random.sample(lines,
                                            round(len(lines) * ratio_list[i][idx]))
                    for line in lines:
                        try:
                            ...
                            label = substr[1]
                            img_path = os.path.join(self.data_dirs[i], file_name)
                            if not os.path.exists(img_path):
                                raise Exception("{} does not exist!".format(img_path))
                            datas.append((img_path, label))
                        except:
                            self.logger.error(
    ...
    def get_ext_data(self):
        ...
        while len(ext_data) < ext_data_num:
            ...
            data_infor = self.data[file_idx]
            img_path, label = data_infor
            data = {'img_path': img_path, 'label': self.replace_symbol(label)}
            if not os.path.exists(img_path):
                continue
    ...
    def __getitem__(self, idx):
        ...
        data_infor = self.data[file_idx]
        img_path, label = data_infor
        label = self.replace_symbol(label)
        try:
            ...
            self.logger.error(
                "When reading {}, error happened with msg: {}".format(
                    img_path, traceback.format_exc()))
            outs = None
        if outs is None:
